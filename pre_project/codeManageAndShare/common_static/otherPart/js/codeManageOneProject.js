...
$(".OneVersion").click(function(){
    ...
    getCodeChangeDetil.toggle("slow")
    if ($(this).attr("alt")!=1) {
        $(this).attr("alt","1")
        $.post("/selfPart/getCodeCompare?CodeManageSecondNumber",{"thisSecondNumber":thisSecondNumber,"lastSecondNumber":lastSecondNumber},function(allCodeCompareString){
            ...
            for(allCodeCompareIndex in allCodeCompares){
                ...
                if (infoFile!=null) {
                    infoFile = infoFile.toString().replace("temp/0","")
                    infoFile = infoFile.toString().replace("temp/","")
                    if (infoType=="addFile") {
                        infoTypeString = "添加文件"
                        thisInfo = '<div style="color:green;width:100%">'+infoTypeString+'>>>'+infoFile + '</div>'
                    }else if (infoType=="addDir") {
                        infoTypeString = "添加目录"
                        thisInfo = '<div style="color:yellow;width:100%">'+infoTypeString+'>>>'+infoFile + '</div>'
                    }else if (infoType=="delDir") {
                        infoTypeString = "删除目录"
                        thisInfo = '<div style="color:#a70606;width:100%">'+infoTypeString+'>>>'+infoFile + '</div>'
                    }else if (infoType=="delFile") {
                        infoTypeString = "删除文件"
                        thisInfo = '<div style="color:red;width:100%">'+infoTypeString+'>>>'+infoFile + '</div>'
                    }else if (infoType=="changeFile") {
                        infoTypeString = "修改文件"
                        thisInfo = '<div style="color:#6039d7;width:100%">'+infoTypeString+'>>>'+infoFile.split(">")[1] + '</div>'
                    };                
